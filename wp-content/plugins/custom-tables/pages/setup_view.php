...
if ($_POST['saveit'] == $tableid AND $_POST['saveit'] != '') {
	...
	elseif ($_GET['wcttab2'] == 'theader') {
		...
		if (isset($sdo) AND $sdo != '' AND $sdo != '0' AND $this->prem_chk() == true) {
			$wpdb->get_row("UPDATE `".$wpdb->prefix."wct_setup` SET ".$zusatz." WHERE `table_id`='".$tableid."' AND `id`='".mres($sdo)."' LIMIT 1;");
		}
		else {
