...
This plugin is supporting multilanguage with the [qtranslate-x](https://de.wordpress.org/plugins/qtranslate-x/ "qtranslate-x") plugin. 
...
This plugin is supporting multilanguage with the [WPML](https://wpml.org/de/ "WPML") plugin. 
You need to create complete a field for each language (except fallback language):

Example: You have a name and text. Name is in all 3 languages identical, but text should be language sensitive in 3 languages: english, german, french.
Englich should be used as fallback language if the text is not translated.

Following fields need to be created:

name <- nothing special, fallback as no language sensitive fields are available
text <- nothing special, fallback for non existing language (as english)
text_de <- will be used for the language Code DE
text_fr <- will be used for the language Code FR
...
= Update 4.1.1 Error / Missing Premium Feature Addons in Plugin List =

The Addons which came prior to Version 4.1.1 separated, where integrated. They will automatically load themself after the premuim license will be entered.
If you update to the Version 4.1.1 and had the Addons activated, you will get an error message "the plugins do not anymore exists". This is correct and will be only shown once. Please ignore it.
...
= 4.1.0 =
* [Feature] jQuery Validation for Forms added
* [Feature] `date()` and `time()` possible for standard values in forms
* [Feature] Required fields in forms possible
* [Bugfix] TinyMCE Frontendeditor has been fixed
* [Bugfix] Loosing Sort and Order in Backend Table has been fixed
* [Bugfix] Picture didn't show correctly in backend has been fixed
* [Bugfix] Debug Fieldnames of varchar(32) fields disabled in form view again
* [Bugfix] Premium Plugins can be activated on Single installations and MU installations again
...
= 3.9.9 =
* [Feature] Basic WPML Support added in tables, more details please see FAQ

= 3.9.8 =
* [Feature] Filter `urlfilter` added for url generation modify
* [Bugfix] Javafix for adding new fields

= 3.9.7 =
* [Feature] `revrel="1"` Added to enable reverse relation within tables output
* [Feature] `groupby="fielname"` Added to enable grouping functionality

= 3.9.6 =
* [Bugfix] `sort=""` is working in wctable shortcode
...
= 3.9.1 =
* [Feature] New filter `wctoutput` added for table filtering
