...
elseif ($_POST['ts'] == '1') {
	...
	foreach($_POST as $var => $wert) {
		...
		if (substr($var,0,5) == "felb_" AND $wert == '1') {
			$feld3[] = rtrim(substr($var,5,1024));
		}
		elseif (substr($var,0,5) == "valu_") {
			$feld2 .= trim($wert).",";
		}
	}
		
	if ($feld2 != '') { $feld2 = substr($feld2,0,strlen($feld2)-1); }
	if (is_array($feld3)) { $feld3 = implode(",",$feld3); }
	$wpdb->get_row("UPDATE `".$wpdb->prefix."wct_form` SET `t_setup`='".mres($feld)."', `t_val`='".mres($feld2)."', `p_fields`='".mres($feld3)."' WHERE `id`='".mres($formid)."' LIMIT 1;");
}
