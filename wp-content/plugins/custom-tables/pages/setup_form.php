...
if ($_GET['page'] != 'wct_editforms') {
	...
	switch ($wcttab) {
		...
		case 'tsetup':
			echo "<h3>" . __('Table Setup','wct'). "</h3><input type=\"hidden\" name=\"ts\" value=\"1\"><table><tr><td><b>".__('Field','wct')."</b></td><td><b>".__('Show','wct')."&nbsp;</b></td><td><b>".__('Required field','wct')."</b></td><td><b>".__('Standard Value (PHP Variable possible)','wct')."</b></td><td><b>".__('Rights','wct')."</b></td></tr>";
			$myrow = $wpdb->get_row("SELECT `r_fields`,`t_setup`,`t_val`,`r_table`,`p_fields` FROM `".$wpdb->prefix."wct_form` WHERE `id`='".mres($formid)."' LIMIT 1;");			
			...
			$felder3 =  explode(",",$myrow->p_fields);

			if (strstr($myrow->t_val,",")) { $valu = explode(",",$myrow->t_val); } else { $valu= array(); } 
			$t = '0';
			foreach ($treffer as $f => $feld) {
				...
				echo "></td><td><input type=\"checkbox\" value=\"1\" name=\"felb_".$feld."\"";
				if (in_array($feld,$felder3)) { echo " checked"; }
				echo "></td><td><input type=\"text\" name=\"valu_".$feld."\" value=\"".str_replace("\"","&quot;",stripslashes($valu[$t]))."\"></td><td><font color=\"";
