...
switch ($_GET['action']) {
	...
	case 'edit':
		...
		echo $message."<form action=\"".$wctmp.$tableid."&wcttab=content&wcttab2=".$_GET['wcttab2']."&action=edit2&wcts=".$_GET['wcts']."&rid=".$_GET['rid']."\" method=\"post\"><table style=\"min-width:800px;width:90%;\">";
		for($i=1;$i<=$x;$i++) {
			if ($feld[$i] != 'id') {
				$wass = $feld[$i];
				echo "<input type=\"hidden\" name=\"wctff[]\" value=\"".$feld[$i]."\" /><tr><td style=\"width:150px;\"><strong>".$wass."</strong>&nbsp;</td><td>";
				if ($feld2[$i] == "smallint(6)") { echo "<input size=\"7\" type=\"text\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\" maxsize=\"6\"/>"; }
				elseif ($feld2[$i] == "int(11)") { echo "<input size=\"12\" type=\"text\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\" maxsize=\"11\"/>"; }
				...
				elseif ($feld2[$i] == "varchar(32)") { echo "<input size=\"34\" type=\"text\" name=\"wctf[]\" value=\"".htmlspecialchars(stripslashes($qry->$wass), ENT_QUOTES)."\" maxsize=\"32\"/>"; }
				elseif ($feld2[$i] == "varchar(64)") { echo "<input size=\"65\" type=\"text\" name=\"wctf[]\" value=\"".htmlspecialchars(stripslashes($qry->$wass), ENT_QUOTES)."\" maxsize=\"64\"/>"; }
				elseif ($feld2[$i] == "varchar(128)") { echo "<input size=\"112\" type=\"text\" name=\"wctf[]\" value=\"".htmlspecialchars(stripslashes($qry->$wass), ENT_QUOTES)."\" maxsize=\"128\"/>"; }
				elseif ($feld2[$i] == "varchar(160)" OR $feld2[$i] == "varchar(254)") {
					echo "<input id=\"upload_image".$feld[$i]."\" type=\"text\" size=\"90\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\" /><input id=\"upload_image_button".$feld[$i]."\" type=\"button\" value=\"".__('Upload Image','wct')."\" />";
					$picturefeld[] = $feld[$i];
				}
				elseif ($feld2[$i] == "text") {
					echo "<input type=\"hidden\" name=\"wctf[]\" value=\"SpeCial_".$feld[$i]."\"/>";
					the_editor(stripslashes($qry->$wass), "wctfs_".$feld[$i], $feld[($i-1)], $media_buttons = false);
				}
				elseif (substr($feld2[$i],0,4) == "enum") {
					...
					foreach ($defs as $posibility) {
						...
						if ($posibility == stripslashes($qry->$wass)) { echo " selected"; }
				...
				elseif (substr($feld2[$i],0,3) == "set") {
					...
					$wasjetzt = ",".stripslashes($qry->$wass).",";
					...
					if (count($exists2) == '1') {
						...
						if (count($abfrage3) >= '1') {
							...
							foreach ($abfrage3 as $rel => $te) {
								if (stripslashes($qry->$wass) == $te->id) { $done = 1; }
								echo "<option value=\"".$te->id."\" ".(stripslashes($qry->$wass) == $te->id ? " selected" : "").">".$te->$felde.(($te->n2 != $te->$felde AND $te->n2 != '') ? " (".$te->n2.")" : "")."</option>";
							}
							if ($done != '1') { echo "<option value=\"".$qry->$wass."\" selected>--no change--</option>"; }
							echo "</select> <a href=\"admin.php?page=wct_table_".$exists2->t_table."&wcttab=content&action=edit&rid=".$qry->$wass."\" style=\"text-decoration:none;\">[EDIT]</a>";
						}
						else {
				...
				elseif ($feld2[$i] == "float(8.2)") { echo "<input size=\"12\" type=\"text\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\" maxsize=\"11\"/>"; }
				else {
					...
					if (count($abfrage) == '1') {
						if (preg_match("/\(([0-9]*)[,]{0,1}.*\)/",$abfrage->name,$treffer)) {
							echo "<input size=\"".($treffer[1]+1)."\" type=\"text\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\" maxsize=\"".$treffer[1]."\"/>";
						}
						echo "<input size=\"112\" type=\"text\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\"/>";
					}
					else {
						echo __('undefinied field type','wct')."<input type=\"hidden\" name=\"wctf[]\" value=\"".stripslashes($qry->$wass)."\" />";
					}
				}
				echo "</td></tr>";
			}
	...
	<table><tr><td width=\"300\" valign=\"top\"><h2>".__($status,'wct')." ". __('Entries','wct')."</h2></td><td><div style=\"width: 120px;border: 1px dotted #900; background-color:#FFFFD9;\" onclick=\"showhide()\"><center>".__('Show/Hide Filters','wct')."</center></div><div style=\"width: 120px;border: 1px dotted #900; background-color:#FFFFD9;\"><a href=\"".$wctmp.$tableid."&wcttab=content&wcttab2=".$_GET['wcttab2']."&wcts=".$_GET['wcts']."&".$filter2."wct_ss=1\"<center>".__('Show All Fields','wct')."</center></a></div></td></tr></table>
		...
		$r = ceil($anz / 30);
		if ($r >= '2') {
			$sortme = "<tr><td colspan=\"3\"></td><form action=\"".$wctmp.$tableid."&wcttab=content&wcttab2=".$_GET['wcttab2']."&wctsort=".$_GET['wctsort']."&wctinvs=".$_GET['wctinvs']."\" method=\"post\"><td colspan=\"".($i + 1)."\"><div class=\"tablenav\"><div class=\"tablenav-pages\"><span class=\"displaying-num\">".$anz." Elemente</span>
				...
				<a class=\"next-page".($starting >= (($r-1)*30) ? " disabled" : "")."\" title=\"Zur n&auml;chsten Seite gehen\" href=\"".$wctmp.$tableid."&wcttab=content&wcts=".($starting >= (($r-1)*30-30) ? (($r-1)*30) : ($starting+30))."&wcttab2=".$_GET['wcttab2']."&wctsort=".$_GET['wctsort']."&wctinvs=".$_GET['wctinvs']."\">&gt;</a>
			...
			foreach ($qry as $row) {
				...
				for($i=1;$i<=$z;$i++) {
					...
					if ($relationA[$i] != '') {
						...
						$wass = $feld[$i];
						if (is_numeric($row->$wass) AND $row->$wass >= '700000000' AND $row->$wass < '3000000000') { echo date("Y-m-d",$row->$wass); }
						else { echo $row->$wass; }
		...
		echo ($globaledit == '1' ? "<tr><td colspan=\"10\"><input type=\"submit\" name=\"submit\" value=\"". __('Save all Changes', 'wct') ."\"></td></tr>" : "")."</tbody>".($globaledit == '1' ? "</form>" : "");
