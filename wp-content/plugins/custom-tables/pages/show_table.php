...
if(!function_exists('qtrans_getLanguage')){function qtrans_getLanguage(){ return '';}}
	...
	if ($out == false OR is_user_logged_in() OR $searchqry != '') {
		...
		if ($table->t_setup != '') {
			...
			if ($searched != '1' OR $searchqry != '') {
				...
				$out .= "<div class=\"table-responsiv\"><table name=\"wct".$css."-table\" id=\"wct".$css."-table\" class=\"wct".$css."-table\">";
				
				if ($table->headerline == '1') {
					$out .= "<tr class=\"wct".$css."-headline\">";
					$y = '0';
					for ($x=0;$x<$rowcount;$x++) {
						$sorting = explode(",",$table->headersort);
						if ($table->header == '') {
							...
							foreach ($felder as $feld) {
								...
								if ($sorting[$y] != '') { $out .= " ".$this->wct_sort($sorting[$y],$table); }
								$out .= "</td>";
								$y++;
							}
						}
							...
							foreach ($felder as $feld) {
								if (preg_match('/--:'.qtrans_getLanguage().'-->.*?<!--:--.*/',rbr($feld)) AND qtrans_getLanguage() != '') {
									preg_match('/--:'.qtrans_getLanguage().'-->(.*?)<!--:--.*/',rbr($feld), $treffer);
									$feld = $treffer[1];
								}
								elseif (preg_match('/.*--:'.ICL_LANGUAGE_CODE.'-->.*?<!--:--.*/',$inhalt) AND function_exists('icl_get_languages'))
								{
									preg_match('/.*--:'.ICL_LANGUAGE_CODE.'-->(.*?)<!--:--.*/',rbr($feld), $treffer);
									$feld = $treffer[1];
								}
				...
				if ($revrel == '1') {
					$relations = $wpdb->get_results("SELECT * FROM `".$wpdb->prefix."wct_relations` WHERE `t_table`='".$id."';");
					if (count($relations) >= '1') {
						foreach ($relations as $relation) {
							$tr = $wpdb->get_row("SHOW CREATE TABLE `".$wpdb->prefix."wct".$relation->s_table."`;");
							$array=array(); foreach($tr as $member=>$data) { $array[$member]=$data; }
							$felder = explode("PRIMARY KEY",$array['Create Table']);
							$felder = explode(",",str_replace(array("8,2","','","\r","\n"),array("8.2","'.'","",""),$felder[0]));
							for ($i=1;$felder[$i] != '';$i++) {
								$srel .= preg_replace("/.*`(.*)`\s.*/","`".$relation->s_table."`.`$1` AS `$1---".$relation->s_table."`,",$felder[$i-1]);
							}
							if ($alldone[$relation->s_table."-".$relation->t_table] != '1') {
								$alldone[$relation->s_table."-".$relation->t_table] = '1';
								$abfrage2 .= "LEFT JOIN `".$wpdb->prefix."wct".$relation->s_table."` as `".$relation->s_table."` ON `".$relation->t_table."`.`".$relation->t_field."`=`".$relation->s_table."`.`".$relation->s_field."` ";
							}
						}
					}
				}
				...
				$finishabfrage = " FROM `".$wpdb->prefix."wct".$id."` as `".$id."` ". $abfrage2 . "WHERE `".$id."`.`status`='active'".$zusatz.($groupby != '' ? "GROUP BY `".$groupby."` " : "");		
				...
				if (count($qry) >= '1') {
					...
					foreach ($qry as $row) {
						...
						if (has_filter('the_editor', 'qtrans_modifyRichEditor')) {
							...
							/*qtranslate support*/
							if (preg_match('/.*!--:'.qtrans_getLanguage().'-->(.*?)<!--:--.*/',$inhalt) AND qtrans_getLanguage() != '')
							{
							...
							elseif (preg_match('/.*!--:'.ICL_LANGUAGE_CODE.'-->(.*?)<!--:--.*/',$inhalt) AND function_exists('icl_get_languages'))
							{
								preg_match('/.*!--:'.ICL_LANGUAGE_CODE.'-->(.*?)<!--:--.*/',$inhalt, $treffer);
								$tmp = $treffer[1];
						...
						/* replace all {fields} with content, including relations */	// stripslashes(\$row->$2$1)
						global $rowtmp;
						$rowtmp = $row;
						$tmp = stripslashes(
							//preg_replace("/\{([0-9]*)\.*(.*?)\}/e","(\$row->$2$1_de == '' ? stripslashes(\$row->$2$1) : stripslashes(\$row->$2$1_de) )",$tmp)
							preg_replace_callback("/\{([0-9]*)\.*(.*?)\}/", function($m) {
								global $rowtmp;
								$tmp1 = $m[2].$m[1]."_de";
								$tmp2 = $m[2].$m[1];
								return ($rowtmp->$tmp1 == '' ? stripslashes($rowtmp->$tmp2) : stripslashes($rowtmp->$tmp1) );
							}, $tmp)
						);
						unset($rowtmp);
						...
						if ($table->overlay == '1') {
							...
							if (preg_match('/!--:'.qtrans_getLanguage().'-->(.*?)<!--:--/',$this->filter_tables(apply_filters('wct_overlay',$table->o_setup))) AND qtrans_getLanguage() != '') {
								preg_match('/.*!--:'.qtrans_getLanguage().'-->(.*?)<!--:--.*/',$this->filter_tables(apply_filters('wct_overlay',$table->o_setup)), $treffer);
							...
							elseif (function_exists('icl_get_languages') AND preg_match('/!--:'.ICL_LANGUAGE_CODE.'-->(.*?)<!--:--/',$this->filter_tables(apply_filters('wct_overlay',$table->o_setup)))) {
								preg_match('/.*!--:'.ICL_LANGUAGE_CODE.'-->(.*?)<!--:--.*/',$this->filter_tables(apply_filters('wct_overlay',$table->o_setup)), $treffer);
								$overlay .= "<div style=\"z-index:9999;".$pos."\" id=\"wct_overlay".$cache."_".$row->id."\" class=\"wct".$css."-overlay\">".
								"<a style=\"float: right;text-decoration:none;\" href=\"javascript:box_hide2('wct_overlay".$cache."_".$row->id."');\">X</a>".
								rbr(rbr(do_shortcode(stripslashes(preg_replace("/\{(.*?)\}/e","stripslashes(\$row->$1)",$treffer[1]))," ",">","<")))."</div>";
							}
					...
					for ($x=1;$x <= $menge;$x++) {
						...
						if ($ausgabe == '1') {
							if($_GET['wct'.$this->wctnr.'start'] == $l OR ($l == '1' AND $_GET['wct'.$this->wctnr.'start'] == '')) { $out .= $x."&nbsp;"; }
								...
								$out .= "<a class=\"wct-numbs\" href=\"".$url."wct".$this->wctnr."start=".$l."\">".($_GET['wct'.$this->wctnr.'start'] == $l ? "<b>".$x."</b>" : $x )."</a>&nbsp;";
							}
						}
					}
				...
				$out .= "</table></div>";
				$out = apply_filters('finished_table', $out);
